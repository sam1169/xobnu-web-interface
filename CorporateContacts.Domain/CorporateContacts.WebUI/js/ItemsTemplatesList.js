...
function createItemsTemplateGrid(folderID, type, name) {
    $("#noRecordsMessageContainer").hide();
    var vUrl = '../ItemList';
    var vColumnHeaderNames = [
        "First Name", "Last Name", "Company Name", "Job Title", "Mobile", "Business Phone", "Email Address", "ID"
    ];
    jQuery(function ($) {
        ...
        jQuery(gridSelector).jqGrid({
            ...
            gridComplete: function () {
                ...
                for (var i = 0; i < ids.length; i++) {
                    ...
                    var givenName = dataFromTheRow.GivenName;
                    var lastName = dataFromTheRow.LastName;
                    var company = dataFromTheRow.CompanyName;

                    var fid = folderID;
                    var pid = $('#grid-table').getGridParam('page');

                    var rowHtml = "<a href='#modal-form' onclick='editItem(" + emailTemplateId + ", " + fid + ", " + pid + ")' class='blue'>" + givenName + "</a>";
                    $('#grid-table').jqGrid('setRowData', ids[i], { GivenName: rowHtml });

                    var rowHtml1 = "<a href='#modal-form' onclick='editItem(" + emailTemplateId + ", " + fid + ", " + pid + ")' class='blue'>" + lastName + "</a>";
                    $('#grid-table').jqGrid('setRowData', ids[i], { LastName: rowHtml1 });

                    var rowHtml2 = "<a href='#modal-form' onclick='editItem(" + emailTemplateId + ", " + fid + ", " + pid + ")' class='blue'>" + company + "</a>";
                    $('#grid-table').jqGrid('setRowData', ids[i], { CompanyName: rowHtml2 });


                    if (givenName == "" && lastName == "" && company == "")
                    {
                        var rowHtml = "<center><a href='#modal-form' onclick='editItem(" + emailTemplateId + ", " + fid + ", " + pid + ")' class='blue'>View Details</a></center>";
                        $('#grid-table').jqGrid('setRowData', ids[i], { GivenName: rowHtml });
                    }

                    //var rowHtml = "<a href='#modal-form' onclick='editItem(" + emailTemplateId + ", " + fid + ", " + pid + ")' class='blue'>View Details</a>";
                    //$('#grid-table').jqGrid('setRowData', ids[i], { GivenName: rowHtml });
